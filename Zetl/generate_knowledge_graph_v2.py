#!/usr/bin/env python3
"""
Script to generate 1000 interconnected knowledge notes in batches.
...
def generate_batch(idea_type, count, all_fundamentals, all_derived, all_principles, start_idx):
    ...
    for i in range(count):
        ...
        if idea_type == "fundamental":
            ...
            batch.append(idea_name)
            
        elif idea_type == "derived":
            # Pick a random fundamental as parent
            parent_fundamental = random.choice(all_fundamentals)
            ...
            # Generate connections (will be updated after all ideas are created)
            connections = generate_connections("derived", all_fundamentals, [], [], area)
            ...
            # Generate connections (will be updated after all ideas are created)
            connections = generate_connections("principle", all_fundamentals, all_derived, [], area)
...
def main():
    ...
    # Generate fundamental ideas (300) in batches
    print(f"\nGenerating {FUNDAMENTAL_COUNT} fundamental ideas...")
    batch_size = 50
    for batch_start in range(0, FUNDAMENTAL_COUNT, batch_size):
        batch_count = min(batch_size, FUNDAMENTAL_COUNT - batch_start)
        print(f"  Generating batch {batch_start//batch_size + 1} ({batch_count} ideas)...")
        
        batch = generate_batch("fundamental", batch_count, [], [], [], batch_start)
        fundamental_ideas.extend(batch)
    
    # Generate derived ideas (500) in batches
    print(f"\nGenerating {DERIVED_COUNT} derived ideas...")
    for batch_start in range(0, DERIVED_COUNT, batch_size):
        batch_count = min(batch_size, DERIVED_COUNT - batch_start)
        print(f"  Generating batch {batch_start//batch_size + 1} ({batch_count} ideas)...")
        
        batch = generate_batch("derived", batch_count, fundamental_ideas, [], [], batch_start)
        derived_ideas.extend(batch)
    
    # Generate principle ideas (200) in batches
    print(f"\nGenerating {PRINCIPLE_COUNT} principle ideas...")
    for batch_start in range(0, PRINCIPLE_COUNT, batch_size):
        batch_count = min(batch_size, PRINCIPLE_COUNT - batch_start)
        print(f"  Generating batch {batch_start//batch_size + 1} ({batch_count} ideas)...")
        
        batch = generate_batch("principle", batch_count, fundamental_ideas, derived_ideas, [], batch_start)
        principle_ideas.extend(batch)
