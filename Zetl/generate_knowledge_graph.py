#!/usr/bin/env python3
"""
Script to generate 1000 interconnected knowledge notes.
...
# Connection patterns
CONNECTION_PATTERNS = {
    "fundamental_fundamental": 0.3,  # 30% of connections are between fundamentals
    "fundamental_derived": 0.4,      # 40% between fundamental and derived
    "fundamental_principle": 0.2,   # 20% between fundamental and principle
    "derived_derived": 0.3,          # 30% between derived and derived
    "derived_principle": 0.3,        # 30% between derived and principle
    "principle_principle": 0.2,      # 20% between principles
    "cross_area": 0.4                # 40% cross-area connections
}
...
def get_hub_idea(area):
    ...
    if area in HUB_IDEAS:
        return random.choice(HUB_IDEAS[area])[0]
    return None
...
def format_connections(connections):
    """Format connections list for template."""
    result = ""
...
def generate_knowledge_graph():
    """Generate the complete knowledge graph with 1000 notes."""
    ...
    # Now update all files with proper connections
    print("\nUpdating connections in all files...")
    
    # Update fundamental ideas
    for i, idea_name in enumerate(fundamental_ideas):
        area = next((area for area in CORE_AREAS if generate_idea_name("fundamental", area) == idea_name), "game_theory")
        connections = generate_connections("fundamental", fundamental_ideas, derived_ideas, principle_ideas, area, idea_name)
        ...
        # Update connections
        new_content = update_connections(content, connections)
        
        with open(file_path, 'w') as f:
            f.write(new_content)
        
        if (i + 1) % 50 == 0:
            print(f"  Updated {i + 1}/{FUNDAMENTAL_COUNT} fundamental connections")
    
    # Update derived ideas
    for i, idea_name in enumerate(derived_ideas):
        # Find parent fundamental and area
        parent_fundamental = random.choice(fundamental_ideas)
        parent_area = next((area for area in CORE_AREAS if generate_idea_name("fundamental", area) == parent_fundamental), "game_theory")
        
        area = get_area_for_idea("derived", parent_area)
        connections = generate_connections("derived", fundamental_ideas, derived_ideas, principle_ideas, area, idea_name)
        ...
        if (i + 1) % 50 == 0:
            print(f"  Updated {i + 1}/{DERIVED_COUNT} derived connections")
    
    # Update principle ideas
    for i, idea_name in enumerate(principle_ideas):
        area = next((area for area in CORE_AREAS if generate_idea_name("principle", area) == idea_name), "game_theory")
        connections = generate_connections("principle", fundamental_ideas, derived_ideas, principle_ideas, area, idea_name)
        ...
        if (i + 1) % 20 == 0:
            print(f"  Updated {i + 1}/{PRINCIPLE_COUNT} principle connections")
    
    print("\n" + "="*60)
    print("Knowledge graph generation complete!")
    print(f"Generated {FUNDAMENTAL_COUNT} fundamental ideas")
    print(f"Generated {DERIVED_COUNT} derived ideas")
    print(f"Generated {PRINCIPLE_COUNT} principle ideas")
    print(f"Total: {FUNDAMENTAL_COUNT + DERIVED_COUNT + PRINCIPLE_COUNT} notes")
    print("="*60)
