...
for cid in range(NUM_CLUSTERS):
    ...
    # Хабы связаны со всеми мини-хабами и хабами
    for h1 in hubs:
        for h2 in hubs:
            if h1 != h2:
                add_link(h1, h2)
    ...
    # Мини-хабы связаны с 40-60% регулярных узлов
    for mh in mini_hubs:
        targets = random.sample(regulars, min(int(len(regulars) * 0.5), len(regulars)))
        for r in targets:
            add_link(mh, r)

    # Регулярные узлы связаны с 3-5 соседями
    for i, r in enumerate(regulars):
        num_local = random.randint(3, min(5, len(regulars) - 1))
...
for cid1 in range(NUM_CLUSTERS):
    for cid2 in range(cid1 + 1, NUM_CLUSTERS):
        ...
        # Мини-хабы связаны с мини-хабами других кластеров
...
for _ in range(edges_to_add * 2):  # x2 т.к. часть не сработает
    n1 = random.choice(all_node_ids)
    # 70% межкластерных, 30% внутрикластерных
    if random.random() < 0.7:
        candidates = [n for n in all_node_ids if nodes[n]["cluster"] != nodes[n1]["cluster"]]
    else:
        candidates = [n for n in all_node_ids if n != n1 and nodes[n]["cluster"] == nodes[n1]["cluster"]]
    if candidates:
        n2 = random.choice(candidates)
        add_link(n1, n2)
    if sum(link_count.values()) // 2 >= TARGET_EDGES:
        break
