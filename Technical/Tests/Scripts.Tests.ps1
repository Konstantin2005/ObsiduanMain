$technicalRoot = Split-Path -Parent $PSScriptRoot
$repoRoot = Split-Path -Parent $technicalRoot
$scriptRoot = Join-Path $technicalRoot 'Scripts'
. (Join-Path $scriptRoot 'Vault\VaultHelpers.ps1')

function Resolve-Calendula20KFixture {
    $candidates = @(
        (Join-Path $repoRoot 'Calendula-20K'),
        (Join-Path $repoRoot 'Старое\Calendula-20K'),
        (Join-Path $repoRoot 'Calendula-People-Graph-From-Branch\Calendula-20K')
    )

    foreach ($candidate in $candidates) {
        if (Test-Path -LiteralPath (Join-Path $candidate '.obsidian\graph-profiles.json')) {
            return (Resolve-Path -LiteralPath $candidate).Path
        }
    }

    throw "Missing Calendula-20K fixture. Checked: $($candidates -join '; ')"
}

$calendula20KRoot = Resolve-Calendula20KFixture
$calendula20KRootJson = $calendula20KRoot | ConvertTo-Json -Compress
...
"@
            ...
            & (Join-Path $scriptRoot 'Git\daily-push.ps1') -RepoPath $repo -Branch 'develop' -GitPath $fakeGit -LogPath $logPath
...
"@
            ...
            & (Join-Path $scriptRoot 'Git\daily-push.ps1') -RepoPath $repo -GitPath $fakeGit -LogPath $logPath -DryRun
...
"@
            ...
            & (Join-Path $scriptRoot 'Vault\Move-TodayTasks.ps1') -Date '2026-06-10' -KanbanDir $plan
...
"@
            ...
            & (Join-Path $scriptRoot 'Vault\Sort-BoardTasks.ps1') -KanbanDir $plan
...
"@
            ...
            & (Join-Path $scriptRoot 'Vault\collect-mentions.ps1') -VaultPath $root -DiaryRoot $diaryRoot -SocialCapitalRoot $socialRoot
...
"@
    ...
    It 'collect-mentions produces the same result with parallel and serial scans' {
        ...
        try {
            ...
            & (Join-Path $scriptRoot 'Vault\collect-mentions.ps1') -VaultPath $serialRoot -DiaryRoot (Join-Path $serialRoot 'Calendula\Calendula') -SocialCapitalRoot (Join-Path $serialRoot 'Calendula\Соц Капитал') -ThrottleLimit 1
            & (Join-Path $scriptRoot 'Vault\collect-mentions.ps1') -VaultPath $parallelRoot -DiaryRoot (Join-Path $parallelRoot 'Calendula\Calendula') -SocialCapitalRoot (Join-Path $parallelRoot 'Calendula\Соц Капитал') -ThrottleLimit 4
    ...
    It 'Normalize-DayNoteNumbers renames numbered notes and updates references' {
        ...
        try {
            ...
            & (Join-Path $scriptRoot 'Vault\Normalize-DayNoteNumbers.ps1') -VaultPath $root -DiaryRoot $diaryRoot
    ...
    It 'Normalize-DayNoteNumbers fails when a rename target already exists' {
        ...
        try {
            ...
            { & (Join-Path $scriptRoot 'Vault\Normalize-DayNoteNumbers.ps1') -VaultPath $root -DiaryRoot $diaryRoot } | Should Throw
        }
        finally {
            if (Test-Path -LiteralPath $root) {
                Remove-Item -LiteralPath $root -Recurse -Force -ErrorAction SilentlyContinue
    ...
    It 'generate_terms_v2 creates term files in a test vault' {
        $root = New-TempRoot
        try {
            & (Join-Path $scriptRoot 'Vault\generate_terms_v2.ps1') -VaultPath $root -MaxWords 3
    ...
    It 'generate_terms_v2 dry run does not create files' {
        ...
        try {
            ...
            & (Join-Path $scriptRoot 'Vault\generate_terms_v2.ps1') -VaultPath $root -MaxWords 2 -DryRun
    ...
    It 'generate_terms_v2 refuses destructive cleanup without Force' {
        ...
        try {
            ...
            { & (Join-Path $scriptRoot 'Vault\generate_terms_v2.ps1') -VaultPath $root -MaxWords 1 } | Should Throw

            & (Join-Path $scriptRoot 'Vault\generate_terms_v2.ps1') -VaultPath $root -MaxWords 1 -Force
    ...
    It 'sync_leetcode generates problem notes from solved slugs and cache' {
        ...
        try {
            ...
            & (Join-Path $scriptRoot 'Vault\sync_leetcode.ps1') -VaultPath $root -ProblemsDir $problemsDir -CachePath $cachePath -ExistingDir $existingDir -SolvedSlugs @('two-sum')
    ...
    It 'sync_leetcode skips files that already exist and reports zero new files' {
        ...
        try {
            ...
            $result = & (Join-Path $scriptRoot 'Vault\sync_leetcode.ps1') -VaultPath $root -ProblemsDir $problemsDir -CachePath $cachePath -ExistingDir $existingDir -SolvedSlugs @('two-sum', 'two-sum') -PassThru
...
(async () => {
  ...
  const root = __CALENDULA_20K_ROOT__;
  const linkRe = /\[\[([^\]|#]+)(?:#[^\]|]*)?(?:\|[^\]]*)?\]\]/g;
...
'@
            $scriptContent = $scriptContent.Replace('__REPO_ROOT__', $repoRootJson).Replace('__CALENDULA_20K_ROOT__', $calendula20KRootJson)
    ...
    It 'rejects dangerous profiles unless explicitly allowed' {
        ...
        try {
            ...
            Copy-Item -LiteralPath (Join-Path $calendula20KRoot '.obsidian\graph-profiles.json') -Destination (Join-Path $obsidian 'graph-profiles.json')
...
'@
            ...
            $output = & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $scriptRoot 'Obsidian/Set-Calendula20KGraphProfile.ps1') -Profile full-danger -VaultPath $vault 2>&1
            $LASTEXITCODE | Should Not Be 0
            ($output -join [Environment]::NewLine) | Should Match 'requires -AllowDanger'

            $output = & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $scriptRoot 'Obsidian/Set-Calendula20KGraphProfile.ps1') -Profile fast-backbone -VaultPath $vault 2>&1
            $LASTEXITCODE | Should Be 0
...
(async () => {
  ...
  fs.copyFileSync(path.join(__CALENDULA_20K_ROOT__, '.obsidian/graph-profiles.json'), path.join(obsidianRoot, 'graph-profiles.json'));
  ...
  try {
    const Guard = require(path.join(__CALENDULA_20K_ROOT__, '.obsidian/plugins/calendula-graph-guard/main.js'));
...
'@
            $scriptContent = $scriptContent.Replace('__REPO_ROOT__', $repoRootJson).Replace('__CALENDULA_20K_ROOT__', $calendula20KRootJson).Replace('__TEMP_ROOT__', $tempRootJson)
...
(async () => {
  ...
  try {
    const UltraGraphPlugin = require(path.join(__CALENDULA_20K_ROOT__, '.obsidian/plugins/calendula-ultra-graph/main.js'));
...
(async () => {
  ...
  const evidence = require(path.join(root, 'Technical/Scripts/Obsidian/graph-evidence-engine.js'));
...
(async () => {
  const path = require('path');
  const root = __REPO_ROOT__;
  const compiler = require(path.join(root, 'Technical/Scripts/Obsidian/graph-index-compiler.js'));
...
(async () => {
  ...
  const root = __REPO_ROOT__;
  const tempRoot = __TEMP_ROOT__;
  const runtime = require(path.join(root, 'Technical/Scripts/Obsidian/graph-build-runtime.js'));
...
(async () => {
  const path = require('path');
  const root = __REPO_ROOT__;
  const throughput = require(path.join(root, 'Technical/Scripts/Obsidian/graph-throughput-governor.js'));
...
(async () => {
  const path = require('path');
  const root = __REPO_ROOT__;
  const capacity = require(path.join(root, 'Technical/Scripts/Obsidian/graph-capacity-lease-runtime.js'));
...
(async () => {
  ...
  const repoRoot = __REPO_ROOT__;
  const tempRoot = __TEMP_ROOT__;
  const vaultRoot = __CALENDULA_20K_ROOT__;
  const outRoot = path.join(tempRoot, 'graph-store');
  const store = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/build-calendula-graph-store.js'));
...
(async () => {
  const path = require('path');
  const repoRoot = __REPO_ROOT__;
  const governors = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/graph-governors.js'));
...
(async () => {
  ...
  const store = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/build-calendula-graph-store.js'));
  const critical = require(path.join(repoRoot, 'Technical/Scripts/Rendering/graph-critical-frame.js'));
  const queryEngine = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/graph-query-engine.js'));
  const renderPlan = require(path.join(repoRoot, 'Technical/Scripts/Rendering/graph-render-plan.js'));
...
(async () => {
  ...
  const store = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/build-calendula-graph-store.js'));
  const critical = require(path.join(repoRoot, 'Technical/Scripts/Rendering/graph-critical-frame.js'));
  const multiscale = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/graph-multiscale.js'));
  const renderPlan = require(path.join(repoRoot, 'Technical/Scripts/Rendering/graph-render-plan.js'));
...
(async () => {
  ...
  const store = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/build-calendula-graph-store.js'));
  const critical = require(path.join(repoRoot, 'Technical/Scripts/Rendering/graph-critical-frame.js'));
  const worker = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/graph-worker-layer.js'));
  const deepValidation = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/graph-deep-validation.js'));
...
(async () => {
  const path = require('path');
  const repoRoot = __REPO_ROOT__;
  const upgrade = require(path.join(repoRoot, 'Technical/Scripts/Rendering/graph-renderer-upgrade.js'));
...
(async () => {
  ...
  const tempRoot = __TEMP_ROOT__;
  const critical = require(path.join(repoRoot, 'Technical/Scripts/Rendering/graph-critical-frame.js'));
  const stability = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/graph-stability.js'));
...
(async () => {
  ...
  const outRoot = path.join(tempRoot, 'graph-store');
  const store = require(path.join(repoRoot, 'Technical/Scripts/Obsidian/build-calendula-graph-store.js'));
  const { GraphScheduler } = require(path.join(repoRoot, 'Technical/Scripts/Rendering/graph-scheduler.js'));
...
Describe 'Calendula graph benchmark tooling' {
    It 'produces a validated graph performance report on a temp vault' {
        ...
        try {
            ...
            $report = & (Join-Path $scriptRoot 'Obsidian\Measure-CalendulaGraphPerformance.ps1') -VaultPath $vault -StoreRoot $storeRoot -MinimumNodes 3 -NodeBudget 2 -EdgeBudget 2 -PassThru
            ...
            { & (Join-Path $scriptRoot 'Obsidian\Measure-CalendulaGraphPerformance.ps1') -VaultPath $vault -StoreRoot (Join-Path $root 'too-small-store') -MinimumNodes 4 } | Should Throw
        }
        finally {
            if (Test-Path -LiteralPath $root) {
                Remove-Item -LiteralPath $root -Recurse -Force -ErrorAction SilentlyContinue
    ...
    It 'produces a validated evidence engine benchmark report' {
        $output = & node (Join-Path $scriptRoot 'Obsidian\measure-graph-evidence-engine.js') --iterations 1000 --reject-every 7 --edge-every 2 2>&1
    ...
    It 'produces a validated index compiler benchmark report' {
        $output = & node (Join-Path $scriptRoot 'Obsidian\measure-graph-index-compiler.js') --files 1000 --changed 10 --added 3 --deleted 2 --corrupt 4 2>&1
    ...
    It 'produces a validated graph build runtime benchmark report' {
        $output = & node (Join-Path $scriptRoot 'Obsidian\measure-graph-build-runtime.js') --changed-files 1000 --total-files 50000 --changed-mb 100 --workers 6 --event-loop-delay 45 --serialization-ms 900 --unresolved-delta 50 2>&1
    ...
    It 'produces a validated throughput governor benchmark report' {
        $output = & node (Join-Path $scriptRoot 'Obsidian\measure-graph-throughput-governor.js') --workers 2,4,6 --throughput 10000,13800,14200 --event-loop-p95 8,11,24 2>&1
    ...
    It 'produces a validated capacity lease benchmark report' {
        $output = & node (Join-Path $scriptRoot 'Obsidian\measure-graph-capacity-lease-runtime.js') --confidence 0.72 --compiler-backlog-mb 80 --edge-multiplier 12 2>&1
...
(async () => {
  ...
  const fs = require('fs');
  const root = __REPO_ROOT__;
  const create = require(path.join(root, 'Technical/Scripts/Rendering/live-graph/builtin-graph.js'));
  ...
  let savedState = null;
  const vaultRoot = path.join(root, 'Technical', 'vault');
  ...
  const createReloaded = require(path.join(root, 'Technical/Scripts/Rendering/live-graph/builtin-graph.js'));
